...
async function getTodos() {
  let response = await fetch(
    `${API}?q=${inpSearch.value}&_page=${page}&_limit=5`
  ...
  // console.log(Math.ceil(allTodos.length / 4));

  // ? посчитали какой будет последняя страница
  let lastPage = Math.ceil(allTodos.length / 5);
...
// ? элеменеты из модалки для редактирования
